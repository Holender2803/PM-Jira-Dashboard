...
type ReportAudience = 'team' | 'executive' | 'client';
...
function ensureStatusReportsTable() {
    const db = getDb();
    db.exec(`
...
function normalizeAudience(value: unknown): ReportAudience {
...
function collectReportData(): ReportData {
    ...
    const teamConfigRow = db.prepare(`
      SELECT active_engineers, hourly_rate, productive_hours_per_sprint, updated_at
      FROM team_config
      WHERE id = 1
    `).get() as TeamConfigRow | undefined;

    const teamConfigured = Boolean(teamConfigRow);
    ...
    const sprintCost = teamConfigured
...
export async function POST(request: Request) {
    try {
        ...
        const audience = normalizeAudience((body as { audience?: unknown }).audience);
        ...
        const db = getDb();
        db.prepare(`
          INSERT INTO status_reports (id, generated_at, audience, sprint_name, content)
          VALUES (?, ?, ?, ?, ?)
        `).run(id, generatedAt, audience, data.sprint.name, content);
...
export async function GET() {
    try {
        ...
        const rows = db.prepare(`
          SELECT id, generated_at, audience, sprint_name, content
          FROM status_reports
