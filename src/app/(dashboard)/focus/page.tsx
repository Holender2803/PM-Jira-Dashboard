...
export default function FocusPage() {
    ...
    const { setFilters, filters, workflowGroupFilter } = useAppStore();
    const [drawerIssue, setDrawerIssue] = useState<JiraIssue | null>(null);
    const [activeTab, setActiveTab] = useState<'focus' | 'team_load'>('focus');
    const [workloadThreshold, setWorkloadThreshold] = useState(13);
    ...
    const assigneeWorkload = useMemo(
        () => getAssigneeWorkload(issues, { workloadThreshold, groupFilter: workflowGroupFilter }),
        [issues, workloadThreshold, workflowGroupFilter]
    );
    const activeAssigneeId = filters.assignee?.[0];

    const handleSelectAssignee = (assigneeId: string | null) => {
        if (!assigneeId) return;
        setFilters({
            assignee: activeAssigneeId === assigneeId ? undefined : [assigneeId],
        });
    };
        ...
        <div>
            ...
            <div style={{ padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div className="tab-bar">
                    ...
                    <button
                        type="button"
                        className={`tab ${activeTab === 'team_load' ? 'active' : ''}`}
                        onClick={() => setActiveTab('team_load')}
                    >
                        Team Load
                    </button>
                </div>
                ...
                {activeTab === 'team_load' ? (
                    <WorkloadGrid
                        ...
                        onThresholdChange={setWorkloadThreshold}
